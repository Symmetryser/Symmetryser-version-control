...
P=PQ_R_saved.data(:,1)+PQ_S_saved.data(:,1)+PQ_T_saved.data(:,1);
Q=PQ_R_saved.data(:,2)+PQ_S_saved.data(:,2)+PQ_T_saved.data(:,2);
...
P_=[PQ_R_saved.time(logstart:detail:logend),P((logstart:detail:logend),1)];
Q_=[PQ_R_saved.time(logstart:detail:logend),Q((logstart:detail:logend),1)];
...
PHASE_C_T=[PHASE_CONTROL_saved.time(logstart:detail:logend),PHASE_CONTROL_saved.data((logstart:detail:logend),3)];
    ...
    save('plot/P.dat','P_','-ascii');
    save('plot/Q.dat','Q_','-ascii');
